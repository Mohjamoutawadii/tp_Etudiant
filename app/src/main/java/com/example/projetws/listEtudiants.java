...
public class listEtudiants extends AppCompatActivity {
    ...
    private static final String URL_LOAD = "http://192.168.110.213/projet/ws/laodEtudiant.php";
    ...
    private void sendDeleteRequest(Etudiant etudiant) {
        String deleteUrl = "http://192.168.110.213/projet/controller/deleteEtudiant.php?id=" + etudiant.getId();
    ...
    private void sendUpdateRequest(Etudiant etudiant){
        String updateUrl = "http://192.168.110.213/projet/ws/updateEtudiant.php";
        RequestQueue requestQueue = Volley.newRequestQueue(this);
        StringRequest updateRequest = new StringRequest(Request.Method.POST, updateUrl,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        Toast.makeText(getApplicationContext(), "Mise à jour effectue !", Toast.LENGTH_SHORT).show();
                    }
                },
                new Response.ErrorListener() {
                    @Override
